...
function getFintab(array $params = []){
...
function countFintab(array $params = []){
    ...
    $totalFin = 0;
        ...
        $sql ='SELECT COUNT(*) as totalList FROM honda_tab_fin';
        if($search1 or $search2 or $search3){
            $sql .=" WHERE";
        }
        if ($search1){
...
function getRicerca($k){
    ...
    $sql = "SELECT DISTINCT  $k FROM honda_tab_fin ORDER BY $k";
